...
public class HelpCommand {
    ...
    public void execute(GuildMessageReceivedEvent event) {
        ...
        embed.addField("Befehle", "**" + this.discord.getBackendManager().getPrefix(event.getGuild()) + "info** - Hier findest Du Fakten und Informationen zum Bot.\n"
                ...
                + "**" + this.discord.getBackendManager().getPrefix(event.getGuild()) + "vaccinations** - Finde die neusten Daten bezüglich der Herdenimmunität.\n"
                + "**" + this.discord.getBackendManager().getPrefix(event.getGuild()) + "hospital** - Finde die wichtigsten Daten von den Intensivstationen.\n"
                + "**" + this.discord.getBackendManager().getPrefix(event.getGuild()) + "support** - Du benötigst Hilfe? Hier findest du unsere Supportwege.", false);
